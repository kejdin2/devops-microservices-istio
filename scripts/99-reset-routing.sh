...
echo "[99] Reset routing to v1 defaults (clean, deterministic)"

kubectl delete virtualservice -n devops vs-service-a-ab vs-service-c-ab --ignore-not-found

kubectl delete virtualservice -n devops \
  vs-service-a-all-v2 vs-service-b-all-v2 vs-service-c-all-v2 \
  --ignore-not-found
...
echo "Active VirtualServices now:"
